        ...
        >
          ...
          <div slot="action" slot-scope="text,record,index">
            ...
            <a v-else-if="!record.isQuick && record.property == 1">
              <a-icon type="heart" style="font-size:14px" @click="changeStar(2,record,index)"  title="快捷面板"/>
            </a>
          </div>
...
export default {
  ...
  methods: {
    ...
    nextFolder(record, index, sort) {
      ...
      this.whereString = `_and:[{parent_id:{_eq:"${record.id}"}},{createdBy_id:{_eq:"${this.userInfo.id}"}}]`
...
.workplace-self .folderAdd,
.workplace-self .upload,
.workplace-self .delete {
  padding: 5px 10px;
...
.workplace-self .folderAdd:hover,
...
.workplace-self .ant-table-thead > tr > th.ant-table-selection-column,
.workplace-self .ant-table-tbody > tr > td.ant-table-selection-column {
  text-align: center;
  min-width: 30px;
  width: 30px;
}
.workplace-self .ant-table-thead > tr > th.ant-table-column-has-actions.ant-table-column-has-sorters,
.workplace-self .ant-table-thead > tr > th.ant-table-column-has-actions.ant-table-column-has-filters {
